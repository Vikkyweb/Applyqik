...
export default function StatCard({
  ...
  return (
    <div className="flex flex-1 items-center gap-3 rounded-2xl bg-white p-4 ring-1 ring-black/5 sm:gap-4">
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-line-soft">
        <Icon className="h-5 w-5 text-slate" strokeWidth={1.8} />
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline gap-2">
          <p className="font-display text-xl font-bold text-ink sm:text-2xl">
            {value}
            {unit && <span className="ml-0.5 text-xs font-semibold text-slate">{unit}</span>}
          </p>
          {change != null && (
            <span
